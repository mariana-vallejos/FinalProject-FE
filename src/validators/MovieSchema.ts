...
export const movieSchema = Yup.object({
    title: Yup.string().trim().required("Title is required").max(120, "Max 120 Chars please 🫦🫦🫦"),
    description: Yup.string().trim().required("Description is required").max(2000, "Max is 2000 characters"),
    posterUrl: Yup.string().trim().url("Must be a valid URL").max(500, "Sorry pal, limit is 500 chars"),
    year: Yup.number()
